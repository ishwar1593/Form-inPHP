...
$allUsers = $searchQuery ? $user->searchUsers($searchQuery) : $user->showAllUsers();
...
<body style="font-family: Arial, sans-serif; margin: 20px; background-color: #f4f4f4;">
    ...
    <!-- Search Bar -->
    <form method="GET" action="allUsers.php" style="text-align: center; margin-bottom: 20px;">
        <input type="text" name="search" placeholder="Search by ID, Name, or Email" value="<?php echo htmlspecialchars($searchQuery); ?>" style="padding: 10px; width: 300px; border-radius: 4px; border: 1px solid #ccc;">
        <button type="submit" style="padding: 10px 20px; border-radius: 4px; border: none; background-color: #007bff; color: white; cursor: pointer;">Search</button>
    </form>
    ...
    <?php } else { ?>
        <p style="text-align: center; color: #ff0000;">Data not found.</p>
    <?php } ?>
    <script>
